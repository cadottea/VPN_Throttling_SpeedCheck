...
def plot_daily_averages_with_error_bars(df):
    ...
    for connection in grouped["Connection"].unique():
        ...
        # Add text annotations for download speeds
        for date, avg_download in zip(connection_data["Date"], connection_data["avg_download"]):
            plt.text(
                date,
                avg_download,
                f"{avg_download:.2f}",
        ...
        # Add text annotations for upload speeds
        for date, avg_upload in zip(connection_data["Date"], connection_data["avg_upload"]):
            plt.text(
                date,
                avg_upload,
                f"{avg_upload:.2f}",
